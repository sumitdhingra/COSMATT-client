import { Component, OnInit, AfterViewInit, OnDestroy, ElementRef, Input, SimpleChange } from '@angular/core';
...
export class TimeSpentComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnChanges(changes: { [propName: string]: SimpleChange }) {
    if (changes['userId'] && changes['userId']['currentValue'] && changes['userId']['currentValue'] == this.userId) {
      this.userId = changes['userId']['currentValue'];
      if(this.mode === 'overall'){
        this.fetchTimeGraphData();
      }else{
        this.fetchChapterGraphData();
      }
  ...
  plotGraph() {
    ...
    if (this.mode === 'overall') {
      ...
      jQuery.plot($graphContainer, [this.seriesData[this.userId][this.timeMode]], this.flotOptions);
    }
    //  else {
  ...
  getChapterWiseFlotData() {
    let data = [];
    let i = 0;
    for (const chapter of this.chapterWiseData[this.userId]) {
  ...
  fetchChapterGraphData(){
    if(this.chapterWiseData[this.userId] && this.chapterWiseData[this.userId].length > 0){
      this.plotGraphChapterwise();
    }else{
      this.loadingTimeMode = true;
      this.tocService.fetchToc(true, true, true, this.courseDataService.getProductId(), this.userId)
      .then((toc) => {
        let i = 0;
        if (!this.chapterWiseData[this.userId]) {
          this.chapterWiseData[this.userId] = [];
        }
        for (const chapter of toc) {
          ...
          this.chapterWiseData[this.userId].push({
            name: chapter.name,
  ...
  fetchTimeGraphData() {
    ...
    if (this.seriesData[this.userId] && this.seriesData[this.userId][this.timeMode] && this.seriesData[this.userId][this.timeMode].length !== 0) {
      this.plotGraph();
    } else {
      ...
      this.courseDataService.getCourseTimeSpent(
        ...
        'timeseries', this.getStartDate().getTime(),
        this.timeModeList[this.timeMode].interval,
        this.userId
      ).then((timeseries) => {
        if(!this.seriesData[this.userId]){
          this.seriesData[this.userId] = [];
        }
        this.seriesData[this.userId][this.timeMode] = [];
        for (const instance of timeseries.result) {
          
          this.seriesData[this.userId][this.timeMode].push([
  ...
  setYAxisTickDetails(): void {
    const yAxisValueType: YAxisValueType = this.getYAxisValueType(this.seriesData[this.userId][this.timeMode]);

    switch (yAxisValueType) {
      case YAxisValueType.Days:
        this.seriesData[this.userId][this.timeMode] = this.seriesData[this.userId][this.timeMode].map(val => {
          return [val[0], moment.duration(val[1], 'minutes').asDays()];
        });
    ...
    jQuery(this.elementRef.nativeElement).resize((e) => {
      ...
      if (this.SCREEN_DEPENDENT_TIMEMODE.indexOf(this.timeMode) > -1) {
        if (this.isDisplayedOnSmallScreen && currentWindowWidth > this.SMALL_SCREEN_OFFSET) {
          ...
          //Calling fetchTimeGraphData() isntead of plotGraph() in case 
          //if data is already not fetched from API and resize functions called.
          if(this.mode === 'overall'){
            this.fetchTimeGraphData();
          }else{
            this.fetchChapterGraphData();
