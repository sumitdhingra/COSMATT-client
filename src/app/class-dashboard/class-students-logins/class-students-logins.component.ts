...
@Component({
  selector: 'app-class-students-logins',
  templateUrl: './class-students-logins.component.html',
  styleUrls: ['./class-students-logins.component.scss']
})
export class ClassStudentsLoginsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  fetchTimeGraphData() {
    if (this.seriesData[this.timeMode] && this.seriesData[this.timeMode].length !== 0) {
      ...
      //use classAnalyticsService.getStudentsLogins for real data. For now using fake data 
      this.classAnalyticsService.getStudentsLogins(
  ...
  private showTooltip(x, y, contents) {
    ...
    tooltipContent += 'Logins: ' + yAxisValue
